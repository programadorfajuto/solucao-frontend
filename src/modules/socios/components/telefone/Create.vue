      ...
      <div class="col-lg-12">
        <div class="form-group">
          ...
          <div class="input-group">
            ...
            <input type="text" class="form-control" v-model="model.descricao" />
          </div>
        </div>
      </div>
      <div class="col-lg-4">
  ...
  methods: {
    salvar() {
      if (this.method == "create") {
        this.socio.telefones.push(this.model);
        this.$emit("close");
      } else {
        this.model.socio = this.socio.id;
        axios
          .post(constantes.url + "telefones", this.model, {
          ...
          .catch((err) => {
            this.$toasted.error(
              err.data
                ? err.data.mensagem
                : "Não foi possível adicionar este telefone."
            );
            this.$emit("close");
          });
      }
